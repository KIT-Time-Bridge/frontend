...
export default function TextBasedSimilarityPage() {
    ...
    // selectedUser가 변경될 때마다 텍스트 기반 유사도 API 호출
    useEffect(() => {
        if (selectedUser) {
            const fetchSimilarityData = async () => {
                ...
                try {
                    ...
                    // 백엔드 응답 형식: { targetPost: {...}, similarPosts: [{ post: {...}, score: ... }, ...] }
                    if (response.data && response.data.similarPosts) {
                        setSimilarityLists(response.data.similarPosts);
                        console.log('텍스트 기반 유사도 데이터:', response.data.similarPosts);
                    } else {
                        setSimilarityLists([]);
                        console.warn('유사도 데이터 형식이 올바르지 않습니다:', response.data);
                    }
                } catch (error) {
                    console.error('텍스트 기반 유사도 API 호출 중 오류 발생:', error);
        ...
        <div>
            ...
            <div className={styles.container}>
                <div className={styles.descriptionGroup}>
                    <h1 className={styles.title}>텍스트 기반 이미지 유사도 조회</h1>
                    <p className={styles.description}>얼굴 특징 속성(텍스트)을 기반으로 유사한 사용자를 찾아 순위로 확인할 수 있습니다.</p>
                </div>
                        ...
                        {loading ? (
                            ...
                            similarityLists.map((item, index) => (
                                <FaceSimilarityCard 
                                    ...
                                    name={item.post?.missing_name || '이름 없음'}
